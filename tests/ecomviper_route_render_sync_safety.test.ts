...
describe("ecomviper route render sync safety", () => {
  it("does not trigger supplier source sync runner during page render", () => {
    ...
    const duplicateShopifyEditorRoutePath = path.join(
      process.cwd(),
      "app/ecomviper/shopify/products/[productId-or-handle]/page.tsx"
    );
    const duplicateShopifyEditorClientPath = path.join(
      process.cwd(),
    ...
    expect(productEditorClient).not.toContain("https://localhost:3001/api/ecomviper/pdp-intelligence");
