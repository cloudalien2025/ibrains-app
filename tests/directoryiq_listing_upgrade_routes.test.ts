...
const getListingEvaluation = vi.fn(async () => ({
  listing: {
    ...
    raw_json: { description: "Current description.", user_id: "321" },
  },
  evaluation: {
    totalScore: 44,
...
vi.mock("@/app/api/directoryiq/_utils/integrations", () => ({
  ...
  pushUserUpdateToBd: vi.fn(async () => ({ ok: true, status: 200, body: {} })),
}));

vi.mock("@/app/api/directoryiq/_utils/bdApi", () => ({
  bdRequestGet: vi.fn(async () => ({
    ok: true,
    status: 200,
    json: { message: { id: "321", group_desc: "Current description." } },
  })),
}));
