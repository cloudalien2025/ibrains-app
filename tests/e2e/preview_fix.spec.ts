...
test("preview fix renders before/after diff", async ({ page }) => {
  ...
  await expect(page.getByText("Fix Preview")).toBeVisible();
  await expect(page.getByText(/Insertions: 1/i)).toBeVisible();
});
