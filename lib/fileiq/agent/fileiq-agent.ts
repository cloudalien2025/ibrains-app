...
/**
 * FileIQ Claude Agent SDK backbone (Phase 1.0 foundation).
 *
 * This module is the server-only seam between a FileIQ extraction job and the
 * Claude Agent SDK. It builds the agent option contract, declares the tool
 * registry an extraction session is allowed to use, and exposes an explicit
 * `runFileIqExtractionAgent` entrypoint that drives a single `query()` session
 * and captures the agent `session_id` for monitoring/retry.
 *
 * Boundaries (Phase 1.0):
 * - No agent session is started from page render or module import.
 * - `runFileIqExtractionAgent` is invoked only by an explicit caller
 *   (future CLI/API route), never during SSR or client hydration.
 * - The model API key is read exclusively from `ANTHROPIC_API_KEY`.
 */
