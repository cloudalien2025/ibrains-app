...
function collectPdpText(record: ShopifyPdpIntelligenceRecord): string[] {
  ...
  return [
    ...
    record.other_ingredients ?? "",
    ...record.trust_signals,
    ...record.certifications,
    ...(record.dietary_attributes ?? []),
    ...(record.manufacturing_claims ?? []),
    ...(record.warnings_text ?? []),
    ...record.compliance_safe_claims,
    record.comparison_content,
    ...(record.faq ?? []),
    ...(record.buyer_intent_mapping ?? []),
    ...(record.entity_mapping ?? []),
    ...(record.semantic_coverage ?? []),
    record.agentic_selection_notes,
    ...(record.referral_readiness ?? []),
    ...faqLines,
