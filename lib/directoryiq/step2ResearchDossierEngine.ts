...
export function buildStep2SelectionResearchDossierPhase1(input: BuildInput): {
  ...
  const contracts: DossierBackedStep2Contract[] = input.slots.map((entry) => {
    ...
    const topResults = sameSiteEvidence.slice(0, 5).map((item, index) => ({
      title: item.title,
      url: item.url,
      rank: index + 1,
    ...
    const artifact: Step2SupportResearchArtifact = {
      ...
      dominant_format: postType(seed.post_type),
      common_title_patterns: dedupe([`${focusKeyword} guide`, `${focusKeyword} for ${input.listing.listing_title}`]),
      common_headings: dedupe([
        `What to know about ${focusKeyword}`,
        `How ${input.listing.listing_title} fits this decision`,
        "Evidence-backed decision points",
      ]),
      common_entities: dedupe([input.listing.listing_title, input.listing.category, input.listing.location_city]).slice(0, 8),
      common_locations: dedupe([input.listing.location_city, input.listing.location_region]).slice(0, 8),
      common_user_questions: dedupe([
        `Is ${input.listing.listing_title} a good fit for ${focusKeyword}?`,
        `What should I compare before choosing ${focusKeyword}?`,
      ]),
      common_decision_factors: dedupe(["fit", "location", "proof", "availability"]),
  ...
  const dossier: Step2SelectionResearchDossierPhase1 = {
    ...
    owner_key: `${input.listing.listing_source_id}:phase1.v1`,
    listing_identity: {
    ...
    normalized_facts: {
      topic_tokens: dedupe([input.listing.category, input.listing.listing_type, input.listing.listing_title]).map(slugPart).filter(Boolean),
      intent_tokens: dedupe(["comparison intent", "selection intent", "fit validation"]),
