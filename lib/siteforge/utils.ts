...
.sf-page-head{padding:24px 24px 0;color:#1e293b}
.sf-page-head h1{margin:0;font-size:clamp(1.35rem,2.4vw,1.9rem);line-height:1.2}
.sf-page-head p{margin:8px 0 0;color:#475569;font-size:.95rem}
.sf-section{margin:0;padding:64px 24px;border-radius:28px;position:relative;overflow:hidden}
...
.sf-kicker{font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:#0ea5e9;margin:0 0 10px;font-weight:700}
...
.sf-card{background:#fff;border:1px solid #dbeafe;border-radius:16px;padding:18px;box-shadow:0 12px 28px rgba(15,23,42,.08)}
.sf-card h3{margin:0 0 8px;font-size:1rem}
.sf-band-accent{background:linear-gradient(140deg,#0f172a 0%,#1d4ed8 48%,#0ea5e9 100%);color:#e2e8f0}
.sf-band-muted{background:#f8fafc}
.sf-band-contrast{background:#0b1f3a;color:#e2e8f0}
.sf-band-accent .sf-lede,.sf-band-contrast .sf-lede,.sf-band-accent .sf-body,.sf-band-contrast .sf-body{color:#dbeafe}
.sf-transition-accent:before{content:"";position:absolute;inset:-30% auto auto -20%;width:260px;height:260px;background:radial-gradient(circle,#38bdf844,transparent 70%)}
.sf-transition-calm:before{content:"";position:absolute;inset:-20% -8% auto auto;width:220px;height:220px;background:radial-gradient(circle,#0ea5e922,transparent 70%)}
.sf-btn{display:inline-block;margin-top:16px;background:#0284c7;color:#fff!important;text-decoration:none;font-weight:700;padding:12px 18px;border-radius:999px;border:1px solid #0369a1;box-shadow:0 8px 20px rgba(2,132,199,.24)}
.sf-btn.secondary{background:transparent;border:1px solid #94a3b8;color:#e2e8f0!important}
...
.sf-trust-item{padding:14px 15px;border-radius:14px;background:#fff;border:1px solid #cbd5e1;color:#0f172a}
...
.sf-value-list li:before{content:"";position:absolute;left:0;top:.6em;width:7px;height:7px;border-radius:999px;background:#0ea5e9}
.sf-hero-grid{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,.85fr);gap:24px;align-items:center}
.sf-visual-frame{border:1px solid #bfdbfe;background:linear-gradient(165deg,#f8fafc,#dbeafe);border-radius:20px;min-height:230px;display:flex;align-items:center;justify-content:center;padding:18px}
.sf-visual-frame p{margin:0;color:#1e3a8a;font-weight:600;text-align:center;line-height:1.35}
