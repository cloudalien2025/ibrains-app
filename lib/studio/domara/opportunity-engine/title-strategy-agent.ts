...
function buildCandidateSeeds(research: CasaHudOpportunityResearchResult): Array<{ title: string; regionHint: string }> {
  const profile = resolveCasaHudMarketProfile(research.preferredMarket);

  return [
    {
      title: `${profile.roundupCount} Affordable Beachfront Homes in ${profile.roundupRegion}`,
      regionHint: profile.roundupRegion,
    },
    {
      title: `Could You Retire in ${profile.lifestyleRegion} for Under ${profile.budgetLabel}?`,
      regionHint: profile.lifestyleRegion,
    },
    {
      title: `Best ${profile.nicheRegion} ${profile.nichePropertyPlural} Under €1M`,
      regionHint: profile.nicheRegion,
    },
    {
      title: `The Best Homes Near ${profile.locationCity} for ${profile.buyerPersona}`,
      regionHint: profile.locationCity,
    },
    {
      title: `Inside a ${profile.showcaseDescriptor} ${profile.showcaseLocation} ${profile.showcaseProperty} With ${profile.showcaseHook}`,
      regionHint: profile.showcaseLocation,
    },
    {
      title: `${profile.roundupCount + 1} ${profile.roundupRegion} Homes That Cost Less Than a ${profile.comparisonBenchmark}`,
      regionHint: profile.roundupRegion,
    },
  ];
}
...
export function buildCasaHudOpportunityTitleCandidates(
  ...
  return buildCandidateSeeds(research)
    .map((seed) => {
      const campaignType = classifyCampaignType(seed.title);
      const scored = scoreCandidate(seed.title, campaignType, research);
