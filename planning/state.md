...
Last updated: 2026-06-04 (UTC) — FileIQ 2050 dark-theme UI redesign conflict resolution (DELIVERED)
...
- FileIQ Result Parser + Performance Router + Rocktomic CSV Deterministic Path: Completed and merged (`feat/fileiq-result-parser-performance-router`, merge SHA `642bc35`, remote + local branch deleted). Fixes fenced-JSON result parsing bug (job `defb9b8a` evidence: agent returned 154 products inside a markdown fence → stored as `raw`, UI showed `Extracted: 0`). Delivered Tasks A–E: (A) `lib/fileiq/result-parser.ts` — 4-step JSON extraction cascade (direct → fenced block → balanced object → raw), `detectProductCatalogSummary`, `computeExtractedCount`; (B) structured timing instrumentation in worker — `agentDurationMs`/`parseDurationMs`/`insertDurationMs`/`totalMs` logged + stored in `job.summary._timing`; (C) `lib/fileiq/performance-router.ts` — classifies jobs by file type into route + maxTurns (CSV→deterministic/0, XLSX→hybrid/8, PDF→agent/20, bundle>3→chunked/40); `lib/fileiq/csv/rocktomic-inventory-parser.ts` — deterministic Rocktomic inventory CSV parser (category headers, status/access normalization, ETA→`replenishmentEta`, reportDate→`lastUpdated`); worker CSV fast path skips agent entirely when confidence is high; (D) generic extracted-count by schema type; (E) 115 new tests pass (result parser 20, performance router 19, CSV parser 27, worker updated). Prompt hardened with explicit "Output ONLY valid JSON — no prose, no markdown fences" rules. **Post-deploy action required: `systemctl restart fileiq-worker` on production to pick up result parser, router, CSV fast path, and timing.**
- FileIQ Deterministic CSV Preparse + Agent Orchestration Correction: Completed and merged (`feat/fileiq-deterministic-preparse-agent-orchestration`, merge SHA `6c90ece`, remote + local branch deleted). Architecture correction: Claude Agent SDK is now always the canonical executor for `deterministic_structured` jobs. Replaced the "skip agent entirely when CSV confidence is high" fast path with a two-phase approach — (1) local deterministic preparse (`preparseMs`, `CsvPreparseResult`), (2) compact Claude validation pass (`maxTurns=3`, prompt embeds `PREPARED EVIDENCE` JSON, tells Claude not to reread source file). Router updated: CSV `maxTurns: 0→3`, rationale no longer says "no agent needed". Job summary gains `agentOrchestration: true`, `localPreparse: { parser, productsParsed, confidence }`, `_timing.preparseMs` + `_timing.agentDurationMs`. Graceful fallback: if agent throws or returns non-completed, deterministic payload stored and job completes. 8 new tests added; 81/81 FileIQ tests pass; 0 pre-existing regressions. **Post-deploy action required: `systemctl restart fileiq-worker` on production to pick up the new two-phase orchestration.**
- FileIQ Full CSV Validation Count/Duration Hotfixes: Completed and merged (`fix-fileiq-preparse-full-evidence`, MR `!335`, merge SHA `0018e4e`; `fix-fileiq-compact-validation-contract`, MR `!336`, merge SHA `5f5c7b1`; both MR pipelines succeeded; remote source branches auto-deleted by GitLab; local source branches deleted). Root issue: high-confidence Rocktomic CSV preparse found all `164` products, but the first compact validation prompt allowed Claude to return a one-product catalog; preserving full nested catalog output then made the full 164-product validation run exceed the 60s target. Delivered correction: Claude now receives every parsed product row as compact structured `PREPARED EVIDENCE.products`, returns a small validation object with `validatedProductCount`, and the worker persists the complete deterministic `product_catalog` v1.1 payload with agent validation metadata. `deterministic_structured` routing and `maxTurns=3` were unchanged. Focused checks passed: `npx vitest run tests/fileiq_worker.test.ts tests/fileiq_performance_router.test.ts tests/fileiq_rocktomic_csv_parser.test.ts` (`97/97`). Production deployed and verified after `systemctl restart fileiq-worker.service`: service `active/running`, `NRestarts=0`; fresh Rocktomic `audit-master.csv` job `5d0b4cdb-638c-4354-9673-9faab469d91c` completed with route `deterministic_structured`, confidence `high`, maxTurns `3`, `agentDurationMs=3700`, `totalProductsFound=164`, raw payload `schemaVersion=1.1`, raw payload product count `164`.
- FileIQ URL Source SDK+Firecrawl Fallback + Supplier Detection: Completed and merged (`feat/fileiq-url-source-supplier-detection`, MR `!338`, pipeline `2576463003` success, merge SHA `c80a28216bf1754ef4812bf71094f9732aaf8e73`; remote source branch deleted by GitLab; local source branch deleted). Delivered URL source path: `lib/fileiq/sources/url-source.ts` fetches URL content via Claude Agent SDK `WebFetch` first, quality-checks content length/text-vs-tags/empty shell/paywall signals, and falls back to Firecrawl only when needed; missing `FIRECRAWL_API_KEY` is a startup warning and a clear runtime error only when fallback is required. Ingest now stores fetched URL markdown as normal source content with `fetchMethod`, `sourceRef`, `sourceRefs`, and URL provenance in source/job metadata. Supplier detection now reads `supplier.name`/`supplierName` from completed product_catalog payloads into job summary, infers Rocktomic at ingest from filename/content hints (`rocktomic`, `ROC` SKU prefix), emits Rocktomic deterministic CSV payloads as `Rocktomic Labs LLC`, backfills completed unknown-supplier jobs from latest raw payloads, and exposes a Supplier column in FileIQ job tables. Focused checks passed: `npx vitest run tests/fileiq_url_source.test.ts tests/fileiq_phase_1_2_ingestion.test.ts tests/fileiq_worker.test.ts tests/fileiq_rocktomic_csv_parser.test.ts tests/fileiq_performance_router.test.ts` (`131/131`) and targeted ESLint passed. Broader `tests/fileiq*.test.ts` result: `175/178`, with the 3 failures matching pre-existing stale Phase 1.1 source-registry assertions. `npx tsc --noEmit --pretty false` still fails on the known repo-wide test type baseline; filtered compiler output for changed FileIQ paths was empty. Final local branch/status after closure update: `main`, clean and tracking `origin/main`. Recommended next sprint remains `Dead Export & Import Cleanup (EcomViper + Brains)`.
- FileIQ 2050 Dark-Theme UI Redesign Conflict Resolution: Completed and merged (`feat/fileiq-ui-2050-redesign`, MR `!339`, pipeline `2577211981` success, merge SHA `e49f21919ab74851100215e3737050c243902fc1`; remote source branch deleted by GitLab; local source branch deleted). Root cause: the redesign branch was behind current `main` after fast-moving FileIQ backend/worker/parser/schema work, especially URL source fallback, supplier detection, deterministic preparse + Claude validation orchestration, result parser, performance router, and product_catalog v1.1 updates. Resolution approach: merged latest `origin/main` into the MR branch, preserved current main for FileIQ backend/API/worker/schema/parser/tests, and manually resolved only `app/fileiq/_components/fileiq-workspace-shell.tsx` so the 2050 dark command-center UI still uses real `/api/fileiq/ingest` and `/api/fileiq/jobs` data. Final MR diff against `origin/main` remained UI-only: `app/fileiq/_components/fileiq-workspace-shell.tsx`, `app/fileiq/layout.tsx`, `app/layout.tsx`. UI integration retained upload/dropzone ingest, URL ingest, intent submission, recent jobs polling, schema/extracted count display, supplier display, validation status display, duration, and responsive desktop/mobile grid behavior. Checks: `npx eslint app/fileiq/_components/fileiq-workspace-shell.tsx app/fileiq/layout.tsx app/layout.tsx` passed with one warning for the intentional Google Fonts link in `app/layout.tsx`; focused FileIQ checks excluding the stale Phase 1.1 source-registry assertions passed (`npx vitest run tests/fileiq_phase_1_2_ingestion.test.ts tests/fileiq_worker.test.ts tests/fileiq_url_source.test.ts tests/fileiq_rocktomic_csv_parser.test.ts tests/fileiq_performance_router.test.ts tests/fileiq_result_parser.test.ts tests/fileiq_foundation.test.ts`, `163/163`); broader FileIQ run including `tests/fileiq_source_registry.test.ts` was `175/178` with the same 3 stale Phase 1.1 assertions already documented; `bash scripts/check_route_signatures.sh` passed; GitLab verify-equivalent test list passed (`62/62`); `npm run build` passed; `npx tsc --noEmit --pretty false` still fails on the known repo-wide test type baseline. Production deploy note: no manual production deploy was run during conflict resolution; after main is green, production may be synced with the established commands (`git fetch origin main`, `git reset --hard origin/main`, `npm ci --omit=dev`, restart `ibrains-app.service` and `fileiq-worker`) and `/fileiq` should be smoke-checked with a small safe ingest plus Recent Jobs/worker status verification.
- FileIQ 2050 Ingest Request-Body Fix: In progress (`fix/fileiq-2050-ingest-request-body`, branch pushed, MR open at https://gitlab.com/cloudalien-technologies/ibrains-app/-/merge_requests/new?merge_request%5Bsource_branch%5D=fix%2Ffileiq-2050-ingest-request-body). Root cause: `DEFAULT_BODY_CLONE_SIZE_LIMIT = 10 MB` in `body-streams.js` truncates the multipart body via `cloneBodyStream()` called by Node.js middleware before the route handler reads it; PDFs > 10 MB arrive with an incomplete boundary and `request.formData()` throws. Fix: `experimental.proxyClientMaxBodySize: "100mb"` in `next.config.mjs`. Also: `sanitizeFileName()` for disk-safe upload paths, improved formData catch logging (content-type/content-length/error), improved UI error message, 4 new tests. Targeted FileIQ checks: `29/29` (`fileiq_phase_1_2_ingestion.test.ts`); broader FileIQ run `179/182` (3 pre-existing stale Phase 1.1 source-registry failures unrelated). Files changed: `next.config.mjs`, `app/api/fileiq/ingest/route.ts`, `tests/fileiq_phase_1_2_ingestion.test.ts`. **Post-merge production validation: `git reset --hard origin/main && npm ci --omit=dev && npm run build && systemctl restart ibrains-app.service && systemctl restart fileiq-worker`; confirm `Request body exceeded 10MB` warning no longer appears in `app.log` after PDF upload.**
- Current recommended sprint: `Dead Export & Import Cleanup (EcomViper + Brains)` — QUEUED and ready for a builder (see sprint pack above), then `Test-Only Orphan Modules` and `Stale DirectoryIQ Docs Audit`. Main is clean at `dae0931`.

## Sprint Closure Update: FileIQ 2050 Dark-Theme UI Redesign Conflict Resolution

- Branch: `feat/fileiq-ui-2050-redesign`
- MR: `!339` (https://gitlab.com/cloudalien-technologies/ibrains-app/-/merge_requests/339)
- Date: `2026-06-04 (UTC)`
- Status: `DELIVERED` — MR pipeline `2577211981` success; merge SHA `e49f21919ab74851100215e3737050c243902fc1`; remote source branch deleted by GitLab; local source branch deleted; local `main` reset to `origin/main`.
- Root cause: MR branch `feat/fileiq-ui-2050-redesign` was stale and 12 commits behind `main` after FileIQ backend/worker/parser/schema changes landed. GitLab blocked merge because `app/fileiq/_components/fileiq-workspace-shell.tsx` conflicted with current main's job-summary UI additions.
- Conflict resolution approach:
  - Merged `origin/main` into the MR branch to preserve branch history and main's backend work.
  - Kept current `main` for FileIQ backend/extraction infrastructure: `app/api/fileiq/**`, `lib/fileiq/agent/**`, `lib/fileiq/csv/**`, `lib/fileiq/schema/**`, `scripts/fileiq-worker.ts`, DB helpers, parser/router/source/supplier-detection code, and their tests.
  - Preserved the UI redesign for the FileIQ command center while integrating current real-data fields from main.
- Files changed relative to `origin/main`:
  - `app/fileiq/_components/fileiq-workspace-shell.tsx` — dark full-page FileIQ command center; real ingest/jobs API integration; supplier, schema, extracted count, validation, duration, and relative submitted time in the extraction stream.
  - `app/fileiq/layout.tsx` — standalone FileIQ layout while keeping `ConfiguredClerkProvider` compatibility.
  - `app/layout.tsx` — Google font preconnect/link tags for the redesigned FileIQ surface.
- Verification:
  - `npm ci` — passed.
  - `npx eslint app/fileiq/_components/fileiq-workspace-shell.tsx app/fileiq/layout.tsx app/layout.tsx` — passed with one `@next/next/no-page-custom-font` warning for the intentional root Google Fonts link.
  - `npx vitest run tests/fileiq_phase_1_2_ingestion.test.ts tests/fileiq_worker.test.ts tests/fileiq_url_source.test.ts tests/fileiq_rocktomic_csv_parser.test.ts tests/fileiq_performance_router.test.ts tests/fileiq_result_parser.test.ts tests/fileiq_foundation.test.ts` — passed (`163/163`).
  - Broader FileIQ run including `tests/fileiq_source_registry.test.ts` — `175/178`; failures are the pre-existing stale Phase 1.1 source-registry assertions expecting 2 ready nav items and `migrationState=source_registry` while current main is already at Extraction Jobs.
  - `bash scripts/check_route_signatures.sh` — passed.
  - GitLab verify-equivalent test command from `.gitlab-ci.yml` — passed (`62/62`).
  - `npm run build` — passed.
  - `npx tsc --noEmit --pretty false` — still fails on the known repo-wide test type baseline; no changed FileIQ UI files were implicated.
- Pipeline:
  - MR pipeline: `2577211981`, `success`, https://gitlab.com/cloudalien-technologies/ibrains-app/-/pipelines/2577211981
  - Branch pipeline: `2577211951`, `success`, https://gitlab.com/cloudalien-technologies/ibrains-app/-/pipelines/2577211951
- Production deploy instructions:
  - Do not deploy until main is green.
  - Established sync/restart commands remain:
    ```bash
    cd /root/ibrains-app
    ...
    ```
  - Post-deploy smoke: open `/fileiq`, submit a small safe file if appropriate, confirm Recent Jobs loads, and confirm `fileiq-worker` is not crash-looping.
