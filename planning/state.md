...
- Current recommended sprint: Deploy production build + `systemctl restart fileiq-worker` (picks up fenced-JSON fix, CSV fast path, timing). Then `Dead Export & Import Cleanup (EcomViper + Brains)` — QUEUED and ready for a builder (see sprint pack above), then `Test-Only Orphan Modules` and `Stale DirectoryIQ Docs Audit`. Main is clean at `642bc35`.
