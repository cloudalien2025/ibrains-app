...
Last updated: 2026-06-04 (UTC) — FileIQ result parser + performance router + Rocktomic CSV deterministic path (DELIVERED)
...
- Current recommended sprint: `systemctl restart fileiq-worker` on production (picks up deterministic+agent two-phase path). Then `Dead Export & Import Cleanup (EcomViper + Brains)` — QUEUED and ready for a builder (see sprint pack above), then `Test-Only Orphan Modules` and `Stale DirectoryIQ Docs Audit`. Main is clean at `6c90ece`.
