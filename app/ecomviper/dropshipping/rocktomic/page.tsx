...
export default async function RocktomicDropshippingPage({ searchParams }: RocktomicDropshippingPageProps) {
  const sourceConfig = getRocktomicSourceConfigSnapshot();
  const products = listRocktomicSupplierProducts();
  const params = await searchParams;
  const sku = (params.sku || "").trim();
  const lookup = sku ? lookupRocktomicSupplierProductBySku(sku) : null;
      ...
      <div className="mx-auto max-w-6xl space-y-4">
        <header className="rounded-2xl border border-[#D9E4F0] bg-white/95 p-6">
          ...
          <p className="mt-2 text-sm text-[#475569]">
            Platform-managed supplier references and SKU lookup foundation. Merchants do not upload Rocktomic files in normal workflow.
          </p>
          <div className="mt-4 flex flex-wrap gap-3 text-sm">
            <Link href="/ecomviper" className="text-[#1D4ED8] hover:underline">
              Back to EcomViper
            </Link>
            <Link href="/ecomviper/shopify" className="text-[#1D4ED8] hover:underline">
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <article className="rounded-2xl border border-[#D9E4F0] bg-white/95 p-5">
            ...
            <p className="mt-2 text-sm text-[#475569]">Configured references: {sourceConfig.configuredReferenceCount}</p>
            <p className="text-sm text-[#475569]">Pending references: {sourceConfig.pendingReferenceCount}</p>
            <p className="text-sm text-[#475569]">Product count: {products.length}</p>
            <p className="text-sm text-[#475569]">Last source sync: {asIso(sourceConfig.lastSyncedAt)}</p>
          </article>
          ...
          <article className="rounded-2xl border border-[#D9E4F0] bg-white/95 p-5">
            ...
            <form className="mt-2 flex gap-2" method="get">
              <input
                name="sku"
                defaultValue={sku}
                placeholder="Search SKU (e.g. ROC817)"
            ...
            {lookup ? (
              ...
              <p className="mt-3 text-sm text-[#475569]">Run a SKU lookup to test exact Rocktomic matching.</p>
            )}
          </article>
        </section>
        ...
        <section className="rounded-2xl border border-[#D9E4F0] bg-white/95 p-5">
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {sourceConfig.references.map((reference) => (
        ...
        <section className="rounded-2xl border border-[#D9E4F0] bg-white/95 p-5">
          <h2 className="text-base font-semibold text-[#0F172A]">Sync Placeholder / Logs</h2>
          <p className="mt-2 text-sm text-[#475569]">
            Incremental Rocktomic sync jobs, inventory/pricing/policy feed ingestion, and durable sync audit logs are planned for a later sprint.
          </p>
        </section>
      </div>
    </main>
