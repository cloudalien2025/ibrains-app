...
function intentToProjectName(prompt: string): string {
  const cleaned = prompt
    ...
    .slice(0, 4)
    .join(" ");
  return normalizeNewProjectName(cleaned ? `${cleaned} Site` : "Intent PageBolt Project") ?? "Intent PageBolt Project";
}
...
export default function SiteForgeAppPage() {
  ...
  const [projectName, setProjectName] = useState("PageBolt Project");
  const [persistedProjectName, setPersistedProjectName] = useState("PageBolt Project");
  ...
  const storageStatusMessage =
    storageSummary?.persistenceHealth === "unavailable"
      ? "Persistent storage unavailable. PageBolt is disabled until database storage is restored."
      : storageSummary?.persistenceHealth === "degraded"
        ? "PageBolt is running in memory mode (development/test only). Projects are not durable."
  ...
  function resetWorkspaceState() {
    ...
    setNewProjectName("");
    setProjectName("PageBolt Project");
    setPersistedProjectName("PageBolt Project");
  ...
  async function ensureProjectForIntent(intent: string): Promise<string | null> {
    ...
    const payload = await fetchJson<unknown>("/api/siteforge/projects", {
      method: "POST",
      body: JSON.stringify({ name: generatedName, description: "Persistent PageBolt workspace" }),
    });
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setUserError(err instanceof Error ? err.message : "Failed to load PageBolt projects.", "load");
      }
    })();
  ...
  async function createProject() {
    ...
    try {
      const payload = await fetchJson<unknown>("/api/siteforge/projects", {
        method: "POST",
        body: JSON.stringify({ name: createName, description: "Persistent PageBolt workspace" }),
      });
      const normalized = normalizeProjectPayload(payload);
      if (!normalized) throw new Error("Project create response was invalid.");
  ...
  async function connectAndBegin() {
    ...
    try {
      ...
      if (!targetProjectId) {
        targetProjectId = await ensureProjectForIntent(intentPrompt.trim() || "PageBolt project");
      }
      if (!targetProjectId) return;
    ...
    <div className="px-1 py-1 md:px-2" data-testid="siteforge-workspace-content">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-xs uppercase tracking-[0.2em] text-[#2563EB]">PageBolt</div>
