...
let loggedCredentialDecryptFallback = false;

function logCredentialDecryptFallback(error: unknown): void {
  if (loggedCredentialDecryptFallback) return;
  loggedCredentialDecryptFallback = true;
  const message = error instanceof Error ? error.message : String(error);
  console.warn(
    `[directoryiq/integrations] Failed to decrypt saved integration credential; falling back to env defaults when available. ${message}`
  );
}
...
export async function getDirectoryIqOpenAiKey(userId: string): Promise<string | null> {
  try {
    ...
    logCredentialDecryptFallback(error);
  }
...
export async function getDirectoryIqBdConnection(userId: string): Promise<{
  ...
  let row;
  try {
    row = await getDirectoryIqIntegrationSecret(userId, "brilliant_directories");
  } catch (error) {
    logCredentialDecryptFallback(error);
    return null;
  }
...
export async function pushUserUpdateToBd(params: {
  baseUrl: string;
  apiKey: string;
  userId: string;
  ...
  const form: Record<string, unknown> = {
    ...
    [params.descriptionField]: params.description,
  };
