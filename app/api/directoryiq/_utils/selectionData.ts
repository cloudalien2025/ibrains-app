...
type ListingUpgradeRow = {
  ...
  bd_response_excerpt: string | null;
  created_at: string;
...
export async function hasDirectoryIqSiteConnected(userId: string): Promise<boolean> {
  ...
  if (row) {
    ...
    if (typeof base === "string" && base.trim().length > 0) return true;
  }
...
export async function markListingUpgradePushed(params: {
  ...
  bdResponseExcerpt: string | null;
}): Promise<void> {
  await query(
    `
    UPDATE directoryiq_listing_upgrades
    SET status = 'pushed', pushed_at = now(), bd_update_ref = $4, bd_status = $5, bd_response_excerpt = $6
    WHERE user_id = $1 AND listing_source_id = $2 AND id = $3
    `,
    [params.userId, params.listingId, params.draftId, params.bdUpdateRef, params.bdStatus, params.bdResponseExcerpt]
  );
}
