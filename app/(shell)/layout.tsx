...
export default async function ShellLayout({ children }: { children: ReactNode }) {
  ...
  const userName =
    typeof user.name === "string" && user.name.trim().length > 0 ? user.name : "Unknown user";
  const userRole =
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-6">
          <header className="flex flex-wrap items-center justify-between gap-4 rounded-[28px] border border-white/10 bg-white/5 px-6 py-4 shadow-[0_30px_70px_rgba(2,6,23,0.5)]">
            ...
            <div className="flex items-center gap-3 text-sm text-slate-300">
              <div className="hidden items-center gap-2 rounded-full border border-cyan-300/25 bg-cyan-400/10 px-3 py-1 text-xs text-cyan-100 md:flex">
                <span className="h-2 w-2 rounded-full bg-cyan-300" />
                {userName}
                <span className="text-cyan-200/80">({userRole})</span>
              </div>
