...
const CONNECTION_INSTRUCTIONS: Record<ConnectionInstructionsProvider, InstructionsContent> = {
  ...
  shopify: {
    ...
    whatYouNeed: [
      "The exact installed store domain in the form `your-store.myshopify.com`.",
      ...
      "App scopes: `read_products` and `read_product_listings` if your app uses listing reads.",
    ],
    steps: [
      "Go to Shopify Dev Dashboard.",
      "Create or select your EcomViper app.",
      "Add required scopes: `read_products` and `read_product_listings` if your app uses it.",
      "Create and release an app version.",
      "Install the released app on the exact Shopify store.",
      "Open app Settings, then copy Client ID and reveal/copy Client Secret.",
      "Do not use the App automation token.",
      "Use the exact store `myshopify.com` domain used during install (for example `9c0d01.myshopify.com`).",
    ...
    commonIssues: [
      "App was created but not installed on this exact store.",
      "App version was created but not released.",
      "New scopes were added but the store has not installed/approved the new version.",
      "Wrong `myshopify.com` domain was used.",
      "Public custom domain was used instead of `myshopify.com`.",
      "Missing `read_products` or `read_product_listings` scope.",
      "App automation token was used instead of Client Secret.",
      "Client Secret was copied incorrectly, with missing characters/whitespace, or was rotated.",
      "Credentials were rotated but not re-saved in EcomViper.",
