...
export default function ListingOptimizationClient() {
  ...
  const [slotFieldErrors, setSlotFieldErrors] = useState<Record<number, SlotFieldErrors>>({});
  ...
  const [authorityNetworkDetail, setAuthorityNetworkDetail] = useState<{ mentions: number; linked: number; leaks: number } | null>(null);
  ...
  const [upgradePushedAt, setUpgradePushedAt] = useState<string | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      setFocusTopicBySlot((prev) => {
        const next = { ...prev };
        for (const post of json.authority_posts ?? []) {
          if (next[post.slot] == null) {
            next[post.slot] = post.focus_topic || post.title || `${json.listing.listing_name} local guide`;
          }
        }
        return next;
      });
      ...
      setUpgradePushedAt(null);
      await loadAuthorityNetworkDetail();
    } catch (e) {
  ...
  async function loadAuthorityNetworkDetail() {
    if (!listingId) {
      setAuthorityNetworkDetail(null);
      return;
    }
    try {
      const response = await fetch(
        `/api/directoryiq/authority-network/listings/by-source/${encodeURIComponent(listingId)}`,
        { cache: "no-store" }
      );
      const json = (await response.json()) as { mentions?: number; linked?: number; leaks?: number };
      if (!response.ok) return;
      setAuthorityNetworkDetail({
        mentions: Number(json.mentions ?? 0),
        linked: Number(json.linked ?? 0),
        leaks: Number(json.leaks ?? 0),
      });
    } catch {
      setAuthorityNetworkDetail(null);
    }
  ...
  async function generateDraft(slot: number) {
    ...
    const title = (titleBySlot[slot] ?? "").trim();
    const focusTopic = (focusTopicBySlot[slot] ?? "").trim();

    const nextFieldErrors: SlotFieldErrors = {};
    if (!title) nextFieldErrors.title = "Post title is required before generating a draft.";
    if (!focusTopic) nextFieldErrors.focusTopic = "Focus topic is required before generating a draft.";
    if (nextFieldErrors.title || nextFieldErrors.focusTopic) {
      setSlotFieldErrors((prev) => ({ ...prev, [slot]: nextFieldErrors }));
      setError({
        message: nextFieldErrors.title ?? nextFieldErrors.focusTopic ?? "Draft input validation failed.",
        code: "VALIDATION_ERROR",
        reqId: clientReqId("draft", slot),
      });
      return;
    }
    setSlotFieldErrors((prev) => ({ ...prev, [slot]: {} }));

    setBusyAction(`draft-${slot}`);
    const response = await fetch(`/api/directoryiq/listings/${encodeURIComponent(listingId)}/authority/${slot}/draft/generate`, {
      method: "POST",
    ...
    const json = (await response.json()) as ApiErrorPayload & { research_count?: number };
    if (!response.ok) {
    ...
    const researchCount = Number(json.research_count ?? 0);
    setNotice(
      researchCount > 0
        ? `Draft ready for slot ${slot}. Research: Top ${researchCount} organic results fetched ✅`
        : `Draft ready for slot ${slot}.`
    );
    setBusyAction(null);
    await load();
  }
  ...
  async function generateImage(slot: number) {
    ...
    const focusTopic = (focusTopicBySlot[slot] ?? "").trim();
    if (!focusTopic) {
      setSlotFieldErrors((prev) => ({
        ...prev,
        [slot]: { ...(prev[slot] ?? {}), focusTopic: "Focus topic is required before generating a featured image." },
      }));
      setError({
    ...
    setSlotFieldErrors((prev) => ({ ...prev, [slot]: { ...(prev[slot] ?? {}), focusTopic: undefined } }));

    setBusyAction(`image-${slot}`);
    const response = await fetch(`/api/directoryiq/listings/${encodeURIComponent(listingId)}/authority/${slot}/image/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function publishBlog(slot: number, token: string) {
    ...
    const response = await fetch(`/api/directoryiq/listings/${encodeURIComponent(listingId)}/authority/${slot}/publish`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ approved: true, approval_token: token }),
    });
  ...
  async function pushUpgrade() {
    ...
    setNotice("Pushed to BD");
    setUpgradeState("done");
    setUpgradePushedAt(new Date().toISOString());
  }
      ...
      {detail ? (
        <>
          <ListingHero
            ...
            imageSource={detail.listing.mainImageSource}
            imageResolutionAttempts={detail.listing.imageResolutionAttempts}
          ...
          >
            ...
            <section className="mt-6 rounded-xl border border-white/10 p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {upgradeState === "done" && upgradePushedAt ? (
                  <span className="inline-flex items-center rounded-full border border-emerald-300/40 bg-emerald-400/10 px-3 py-1 text-xs text-emerald-100">
                    Pushed at {new Date(upgradePushedAt).toLocaleString()}
                  </span>
                ) : null}
              </div>
          ...
          <HudCard title="Authority Support" subtitle="Max 4 posts per listing.">
            <div className="mb-3 rounded-xl border border-cyan-300/20 bg-cyan-400/10 p-3">
              <div className="mb-2 text-xs uppercase tracking-[0.08em] text-cyan-100">Authority Network</div>
              <div className="flex flex-wrap gap-2 text-xs text-slate-100">
                <span className="rounded-full border border-white/20 px-2 py-0.5">
                  Mentions: {authorityNetworkDetail?.mentions ?? 0}
                </span>
                <span className="rounded-full border border-emerald-300/40 px-2 py-0.5 text-emerald-100">
                  Linked: {authorityNetworkDetail?.linked ?? 0}
                </span>
                <span className="rounded-full border border-amber-300/40 px-2 py-0.5 text-amber-100">
                  Leaks: {authorityNetworkDetail?.leaks ?? 0}
                </span>
                <a
                  href="/directoryiq/authority-network"
                  className="rounded-full border border-cyan-300/40 px-2 py-0.5 text-cyan-100 hover:bg-cyan-300/10"
                >
                  Fix Missing Links
                </a>
              </div>
              ...
              {detail.authority_posts.slice(0, 4).map((post) => (
                <article key={post.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      setTitleBySlot((prev) => ({ ...prev, [post.slot]: event.target.value }));
                      setSlotFieldErrors((prev) => ({
                        ...prev,
                        [post.slot]: { ...(prev[post.slot] ?? {}), title: undefined },
                      }));
                    }}
                    placeholder="Post title"
                    className="mb-2 w-full rounded-lg border border-white/15 bg-white/[0.04] px-2 py-1.5 text-xs text-slate-100"
                  />
                  {slotFieldErrors[post.slot]?.title ? (
                    <div className="mb-2 text-[11px] text-rose-200">{slotFieldErrors[post.slot]?.title}</div>
                  ) : null}
                  <input
                    value={focusTopicBySlot[post.slot] ?? post.focus_topic ?? ""}
                    onChange={(event) => {
                      setFocusTopicBySlot((prev) => ({ ...prev, [post.slot]: event.target.value }));
                      setSlotFieldErrors((prev) => ({
                        ...prev,
                        [post.slot]: { ...(prev[post.slot] ?? {}), focusTopic: undefined },
                      }));
                    }}
                  ...
                  />
                  {slotFieldErrors[post.slot]?.focusTopic ? (
                    <div className="mb-3 text-[11px] text-rose-200">{slotFieldErrors[post.slot]?.focusTopic}</div>
                  ) : null}
