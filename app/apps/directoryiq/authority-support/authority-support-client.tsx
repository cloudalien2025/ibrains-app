                    ...
                    <td className="py-2 pr-3">
                      <div className="flex flex-wrap gap-2 text-xs">
                        {issue.from?.externalId ? (
                          <Link href={`/apps/directoryiq/authority/blogs?blog=${encodeURIComponent(issue.from.externalId)}`} className="text-cyan-200 underline">
                            Blog
                          </Link>
                        ) : null}
                        {issue.to?.externalId ? (
                          <Link href={`/apps/directoryiq/authority/listings?listing=${encodeURIComponent(issue.to.externalId)}`} className="text-cyan-200 underline">
                            Listing
                          </Link>
                        ) : null}
                      </div>
            ...
            <div className="space-y-3 text-sm text-slate-200">
              ...
              <div className="flex flex-wrap gap-3 pt-1 text-xs">
                {selectedIssue.from?.externalId ? (
                  <Link
                    href={`/apps/directoryiq/authority/blogs?blog=${encodeURIComponent(selectedIssue.from.externalId)}`}
                ...
                {selectedIssue.to?.externalId ? (
                  <Link
                    href={`/apps/directoryiq/authority/listings?listing=${encodeURIComponent(selectedIssue.to.externalId)}`}
                    className="text-cyan-200 underline"
                  >
                    Open Listing Drawer
                  </Link>
