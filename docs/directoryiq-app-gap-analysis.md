...
- Current monorepo implementation: `app/apps/directoryiq/**` and `app/api/directoryiq/**`

## Route parity
- `app/apps/directoryiq/**` now contains the same section structure as standalone.
- Compatibility redirects from `/directoryiq` to `/apps/directoryiq` exist and should remain.

## What was still wrong for product expectations
1. Landing perception mismatch
- User reported `/apps/directoryiq` still looked like an AI-visibility/fetch-failed surface rather than the expected full product experience.
...
- SiteForge app routes remain separate and unchanged.
...
- Ensure `/apps/directoryiq` still reflects real standalone IA while visually matching iBrains.
- Keep `/brains/**` and SiteForge untouched.
